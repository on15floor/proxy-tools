...
def grab_page(num_page):
    ...
    for i in range(0, proxy_amount):
        if i % 2 == 0:
            proxy_ip += [tag_proxy_ip_odd[o].text.split('document')[0]]
            o += 1
            proxy_port += [tag_proxy_port[i].attrs['value']]
        else:
            proxy_ip += [tag_proxy_ip_even[e].text.split('document')[0]]
            e += 1
            proxy_port += [tag_proxy_port[i].attrs['value']]

    delimiter = get_delimiter(proxy_port)
    for i in range(0, proxy_amount):
        proxies += ['{}:{}'.format(proxy_ip[i], proxy_port[i].split(delimiter)[1])]
